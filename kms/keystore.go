...
// Wrapping has been created but is not to be implemented in the first
// pass of work.
/*
...
const (
	// Insecure KeyWrap algoirthms have been disabled.
	// Wrap_AES_ECB WrappingAlgorithm = iota
	// Wrap_AES_CBC
	// Wrap_AES_CTR
	Wrap_PKCS1_OAEP WrappingAlgorithm = iota + 3
)
...
type KeyStore interface {
	...
	GetKeyById(keyId string, password ...string) (Key, error)

	// GetKeyByName searches for the key with the specified name
	GetKeyByName(keyName string, password ...string) (Key, error)
	...
	// SECUROSYS PROPORSAL
	GenerateSecretKey(keyAttributes *KeyAttributes, password ...string) (Key, error)
	...
	// SECUROSYS PROPORSAL
	GenerateKeyPair(keyPairAttributes *KeyAttributes, password ...string) (privateKey Key, publicKey Key, err error)
	GenerateRandom(length int) (random []byte, err error)
